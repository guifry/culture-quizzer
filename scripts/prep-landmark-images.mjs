// Dev-only: source recognisable UK-landmark photos from Wikimedia Commons, screen each
...
//
// Requires OPENAI_API_KEY (read from ./.env or the environment). Model via OPENAI_MODEL
// (default gpt-4o). If no clean, non-spoiling shot is found for a term, that slot is left
// empty rather than shipping a spoiler.
//
// Usage:  node scripts/prep-landmark-images.mjs [landmarkId ...]   (no args = all)
import { mkdir, writeFile, readFile, readdir, rm } from 'node:fs/promises'
import { existsSync, readFileSync } from 'node:fs'
import path from 'node:path'
import sharp from 'sharp'
import { LANDMARKS } from './uk-landmark-photos.mjs'
...
async function main() {
  if (!API_KEY) {
    console.error('OPENAI_API_KEY not found (set it in ./.env or the environment).')
    process.exit(1)
  }
